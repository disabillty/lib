...
local function SaveConfig()
    ...
    for b, meta in pairs(activeToggles) do
        local id = meta.id
        local state = b.Text:sub(1,3)=="[X]"
        data.toggles[id] = state
    end
    for f, meta in pairs(activeSliders) do
        local id = meta.id
        data.sliders[id] = meta.get()
    end
    for b, meta in pairs(activeTextboxes) do
        local id = meta.id
        data.textboxes[id] = b.Text
    end
    for id, key in pairs(keybindData) do
        if type(key)=="string" and key~="" then
            data.keybuttons[id] = key
        end
    end
...
local function LoadConfig()
    ...
    for b, meta in pairs(activeToggles) do
        local id = meta.id
        local st = data.toggles and data.toggles[id]
        if st ~= nil then
            local label = meta.label
            b.Text = (st and "[X] " or "[ ] ")..label
            if meta.cb then meta.cb(st) end
        end
    end
    for f, meta in pairs(activeSliders) do
        local id = meta.id
        local val = data.sliders and data.sliders[id]
        if val ~= nil then meta.set(val) end
    end
    for b, meta in pairs(activeTextboxes) do
        local id = meta.id
        local txt = data.textboxes and data.textboxes[id]
        if txt ~= nil then
            b.Text = txt
            if meta.cb then meta.cb(txt) end
        end
    end
    for b, meta in pairs(activeKeybinds) do
        local id = meta.id
        local saved = data.keybuttons and data.keybuttons[id]
        local kb = b:FindFirstChild("KeyBind")
        if saved and kb then
            kb.Text = saved
            keybindData[id] = saved
...
function L:Window(title,size,pos)
    ...
    h.InputBegan:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.MouseButton1 then dr=true; dx,dy=i.Position.X,i.Position.Y; sx,sy=w.Position.X.Offset,w.Position.Y.Offset end
    end)
    ...
    ui.InputChanged:Connect(function(i)
        if dr and i.UserInputType==Enum.UserInputType.MouseMovement then
            w.Position=UDim2.new(w.Position.X.Scale, sx+(i.Position.X-dx), w.Position.Y.Scale, sy+(i.Position.Y-dy))
        end
    end)
    local o={}
    ...
    function o:KeyButton(t,cbk)
        ...
        kb.Name = "KeyBind"
        kb.Size = UDim2.new(0,30,0,btnH)
        kb.Position = UDim2.new(1,0,0,0)
        kb.AnchorPoint = Vector2.new(1,0)
        kb.BackgroundColor3 = c60
        kb.TextColor3 = wh
        kb.Text = "Key"
        ...
        local conn = ui.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.Keyboard then
                if listening then
                    ...
                    kb.Text = key
                    keybindData[id] = key
                    b:SetAttribute("LoadedKeyBind", key)
                    listening = false
                else
                    local savedKey = keybindData[id]
                    if savedKey and input.KeyCode.Name == savedKey then
                        if cbk then cbk() end
                    end
                end
            end
    ...
    function o:Toggle(t, cbk)
        ...
        local b,st=Instance.new("TextButton"),false
        b.Size=UDim2.new(1,0,0,btnH); b.BackgroundColor3=c60; b.Text="[ ] "..t; b.TextColor3=wh
        b.Font=Enum.Font.SourceSans; b.TextSize=14; b.TextXAlignment=Enum.TextXAlignment.Center; b.Parent=c
        b:SetAttribute("ControlId", id)
        local kb=Instance.new("TextButton"); kb.Name="KeyBind"
        kb.Size=UDim2.new(0,30,0,btnH); kb.Position=UDim2.new(1,0,0,0); kb.AnchorPoint=Vector2.new(1,0)
        kb.BackgroundColor3=c60; kb.TextColor3=wh; kb.Text="Key"; kb.Font=Enum.Font.SourceSans; kb.TextSize=14; kb.Parent=b
        ...
        local conn=ui.InputBegan:Connect(function(input)
            if input.UserInputType==Enum.UserInputType.Keyboard then
                if listening then
                    ...
                    if savedKey and input.KeyCode.Name==savedKey then
                        st=not st
                        b.Text=(st and "[X] " or "[ ] ")..t
                        if cbk then cbk(st) end
                    end
                end
            end
        end)
        ...
        b.MouseButton1Click:Connect(function()
            st=not st; b.Text=(st and "[X] " or "[ ] ")..t; if cbk then cbk(st) end
        end)
        return b
    end
    function o:Textbox(ph,cbk)
        local id = ControlId(winTitle,"Textbox",ph)
        local b=Instance.new("TextBox"); b.Size=UDim2.new(1,0,0,btnH); b.BackgroundColor3=c60; b.PlaceholderText=ph; b.ClearTextOnFocus=false
        b.TextColor3=wh; b.Text=""; b.Font=Enum.Font.SourceSans; b.TextSize=14; b.TextXAlignment=Enum.TextXAlignment.Center; b.Parent=c
        b:SetAttribute("ControlId", id)
    ...
    function o:Slider(t,min,max,cbk)
        ...
        local f=Instance.new("Frame"); f.Size=UDim2.new(1,0,0,btnH); f.BackgroundColor3=c60; f.Parent=c
        f:SetAttribute("ControlId", id)
        ...
        ui.InputChanged:Connect(function(inp)
            if drag and inp.UserInputType==Enum.UserInputType.MouseMovement then
                ...
                val=min+(max-min)*(x/f.AbsoluteSize.X)
        ...
        activeSliders[f]={
            ...
            set=function(v) val=v; l.Text=t.." : "..val; if cbk then cbk(val) end end}
        return f
    end
    return o
end
